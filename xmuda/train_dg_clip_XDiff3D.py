...
from xmuda.XDiff3D.losses import entropy_loss

from xmuda.data.build import build_dataloader
from xmuda.XDiff3D.UniDSeg.build import build_model_2d, build_model_3d
...
def train(cfg, output_dir='', run_name=''):
    ...
    for iteration in range(start_iteration, max_iteration):
        ...
        preds_2d,object_query_2D,object_query_2D_de = model_2d(data_batch_src)
        preds_3d, object_query_3D, x_ori, x_refine = model_3d(data_batch_src,object_query_2D)
        ...
        if cfg.TRAIN.XMUDA.lambda_xm_src > 0:
            ...
            loss_consist = F.kl_div(object_query_2D, object_query_2D_de).sum(1).mean()
            loss_consist.backward()

            x_ori_stack = torch.stack(x_ori)
            x_refine_stack = torch.stack(x_refine)

            loss_sup = criterion(x_ori_stack, x_refine_stack)
            loss_sup.backward()
...
def main():
    ...
    cfg.freeze()
    now = datetime.now()
    cur_filename = now.strftime("%Y%m%d_%H%M%S")
    output_dir = os.path.join(cfg.OUTPUT_DIR, cur_filename)
